...
let selectedWork: Element | null = null;
...
works?.forEach(work => {
  work.addEventListener('click', () => {
    if (selectedWork) selectedWork.classList.remove('selected');
...
const renderQueryParams = (q: QueryParams) => {
  const clearAll = q.mediums?.length == 0 && q.series?.length == 0 && q.taxonomies?.length == 0 && !q.selectedId;
  works?.forEach(el => {
...
window.addEventListener('popstate', (e) => {
  ...
  const mediumsParam = searchParams.get('mediums');
  const selectedId = searchParams.get('id') || undefined;

  const taxonomies = taxonomiesParam ? taxonomiesParam.split(',').map(v => v.trim()) : [];
  const series = seriesParam ? seriesParam.split(',').map(v => v.trim()) : [];
  const mediums = mediumsParam ? mediumsParam.split(',').map(v => v.trim()) : [];

  renderTags({ selectedId, taxonomies, series, mediums });
  renderQueryParams({ selectedId, taxonomies, series, mediums });
})